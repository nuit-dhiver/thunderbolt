...
export default class Database {
  ...
  static async load(path: string): Promise<Database> {
    const _path = await invoke<string>('plugin:libsql|load', {
      db: path,
    })
  ...
  async execute(query: string, bindValues?: unknown[]): Promise<QueryResult> {
    const [rowsAffected, lastInsertId] = await invoke<[number, number]>('plugin:libsql|execute', {
      db: this.path,
      query,
      values: bindValues ?? [],
    })
  ...
  async close(db?: string): Promise<boolean> {
    const success = await invoke<boolean>('plugin:libsql|close', {
      db,
    })
    return success
  }
